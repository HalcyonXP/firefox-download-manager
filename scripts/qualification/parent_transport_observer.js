...
((args) => {
  ...
  const key = "__ownedParentTransportObserverV1";
  const refused = () => {
    ...
    const observer = {
      observe(subject, observedTopic, data) {
        try {
          ...
          if (Object.hasOwn(value, "nonce") && value.nonce !== nonce) return;
          if (
            checking &&
            value.nonce === nonce &&
            subject === null &&
          ...
          if (subject !== null || records.length >= 1) {
            failed = true;
            return;
          }
          // Keep exact original JSON. The external parser rejects duplicates,
          // nonfinite numbers, unknown fields/types, ordering and API-token changes.
          records.push(data);
        } catch {
          failed = true;
        }
