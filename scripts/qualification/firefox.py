...
class Firefox:
    def __init__(self, executable, profile, environment, *, owned_peer=None, fileless_experiment=False, parent_transport_experiment=False):
        ...
        if (type(parent_transport_experiment) is not bool or (parent_transport_experiment
                and (fileless_experiment or type(owned_peer) is not BrowserPeer))):
            raise RuntimeError("parent transport experiment requires its distinct owned companion witness")
        self.parent_transport_experiment = parent_transport_experiment
    ...
    def start(self):
        if self.launch_attempted or self.closed:
            raise RuntimeError("owned Firefox controller already consumed")
        self._require_apps_closed()
        self.profile.mkdir(exist_ok=not (self.fileless_experiment or self.parent_transport_experiment))
        ...
        if self.fileless_experiment or self.parent_transport_experiment:
            # Distinct opt-in experiment variants; profile creation is exclusive.
            # The installed-parent variant additionally requires BrowserPeer.
        ...
        # Experiment variants have only the explicit experiment-capability override.
        ...
        self.automation_policy = observe_policy(self, fileless_experiment=self.fileless_experiment,
                                                parent_transport_experiment=self.parent_transport_experiment)
        return self
