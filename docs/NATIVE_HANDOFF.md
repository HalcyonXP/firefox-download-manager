...
Existing handoffs use task envelope 5: the strictly validated task4 payload plus a required handoff phase at the envelope level. Ordinary records remain task4 and existing v1–v3 migrations remain unchanged. Older engines refuse version5 as incompatible rather than treating Prepared as runnable Queued. This format is independent of wire2. The opt-in companion bridge advertises `prepared_handoff`; the legacy stdio-owned engine neither advertises nor dispatches it. Browser pending-state handling and UI are now implemented as components in [BROWSER_HANDOFF.md](BROWSER_HANDOFF.md), but must be qualified before selecting automatic capture.

The separate opt-in [native protection gate](FIREFOX_PROTECTION_BRIDGE.md#native-publication-gate-opt-in-engine-component) uses envelope6 with an immutable required policy marker. No existing wire preparation selects it. Browser context and decisions remain memory-only; recovered protected records cannot start/retry/resume or newly commit, even after identical re-preparation. Status remains replay-free. Ordinary/previous handoff behavior is unchanged.
...
- Local workspace formatting/Clippy/tests/build, dependency policy, npm checks and privacy screening passed. The nine handoff engine tests and all eight bridge tests also passed the reviewed LLVM/UCRT release target. The first complete workspace run exposed an obsolete future-version fixture: it used ordinary version4 + 1, which is now the supported handoff envelope. The fixture uses the newest supported envelope + 1 (currently protected version6 + 1); malformed version5 remains independently rejected.
