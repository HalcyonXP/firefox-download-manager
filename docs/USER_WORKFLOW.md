...
3. **Install the unsigned XPI in Firefox once**, accepting its normal installation prompts.
4. **Restart Firefox**.
...
Unsupported downloads must stay in Firefox with a clear explanation, not disappear or download twice. The companion must have a clear status and Quit action. The target is a persistent unsigned personal XPI: no signed XPI, Mozilla submission, signing account or credentials are needed. Existing Firefox settings and Firefox/Windows protections remain unchanged. Exact-artifact persistence and the complete browser workflow still require qualification.
