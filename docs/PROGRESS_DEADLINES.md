...
## Follow-up failure: an optional terminal rate was asserted mandatory

CI `34327670669` at `597a6f6` passed the original progress cases and both controlled preparation boundaries. Fixed full-suite observation 2 nevertheless failed the inherited `snapshot.speed_bytes_per_second().is_some()` assertion in the delayed-cadence variant. Its diagnostic was completed 8 MiB, active workers zero, no task failure, four held samples, gate release at 12657 ms and completion at 14367 ms, with terminal speed `None`. Observations 1 and 3 passed; the overall job correctly remained failed and ARM was skipped.

This exposed another test/contract mismatch rather than authorizing a general snapshot waiver. `SpeedEstimator` requires three recent samples. Its existing one-second sliding window can trim previously sufficient history to two endpoints after a gap, yielding `None` even for a completed file; ETA still becomes zero. The protocol's rate field is nullable. The deterministic `completed_sample_after_a_gap_can_have_no_recent_rate` unit test samples at 0/250/500 ms (an actual rate exists), then a completed sample at 2000 ms (rate unavailable, ETA zero). It does not sleep or alter the implementation.

The cadence test still requires an actual `Some` rate in the held-prefix observation window **before releasing work**. The invalid unconditional terminal-`Some` predicate is replaced with a stronger phase-specific history assertion: the rate recorded at entry to `Validating` (including `None`) must be retained through promotion and in the completed event and final snapshot. Completed state, bytes, expected size, ETA zero and exact file assertions remain; zero active workers is additionally explicit. This preserves #25's history behavior without inventing a rate after its sampling window is insufficient.

This specific acceptance correction is recorded in #44; earlier claims of preserving every old snapshot predicate describe the prior checkpoint, not this follow-up. No production rate/persistence/timeout policy changed. Mutations removing window trimming or clearing a known rate during promotion fail the new stale-window unit or phase-history assertion respectively. Original sources were restored; local Clippy and all 34 lifecycle cases plus five estimator tests passed. Final-tip gates remain required. This new failure has an observed terminal field and a deterministic estimator counterexample; it must not be presented as recovering the missing trace of the original two CI timeouts.

### Keep the active rate window consistent with its observation budget

The same review exposed a related premise without waiting for another CI failure: four samples in ten seconds need not include three samples in a one-second sliding window. A deterministic 0/2/4/6-second sample sequence has no rate with the one-second window, but an actual zero rate with a window covering the ten-second observation period. The active integration fixture therefore uses that supported ten-second window while retaining its 100 ms minimum spacing and ten-second observation watchdog. The one-second expiry behavior remains independently tested; production defaults/options are unchanged. This avoids claiming a maximum producer interval or removing the active-window `Some` requirement to obtain a pass.
