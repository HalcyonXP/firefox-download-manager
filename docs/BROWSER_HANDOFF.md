...
Only the owned manager port accepts the closed boolean preference action. It uses an independent queue so an outstanding native action does not delay Off. The checkbox displays unconfirmed/disabled state while delivery or verification is uncertain; a disconnected interface cannot claim that a queued change took effect. Renderer, storage, routing, restart and diagnostic-eligibility models cover these distinctions. Production permission approval/activation, live toggle behavior and exact-XPI persistence remain separate acceptance requirements.
