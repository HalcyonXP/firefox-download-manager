...
1. Bind a native publication challenge to one immutable handoff ID, validation generation, complete 64-bit length, computed SHA-256 and intended final filename. Retain the existing `ValidatedPartial` lock/identity lease while pending; hash computation is presently optional and no reusable protection fingerprint or verdict gate is exposed. Changed bytes, filename collision/reselection, cancellation or a retired lease invalidate the challenge.
