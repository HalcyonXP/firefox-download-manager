...
## #28 actual-browser policy checkpoint (not release approval)

MIT and the existing-machine qualification plan are now settled in ADR0011. The exact licensed CI34341064343 candidate passed the original Firefox slice with a clean37 driver; a later source-hashed draft expands this to twenty actual browser checks, including CSP violations/no inline execution or image/fetch contact, private-window capture and submission refusal, validation cancellation, durable-prefix restart, and renewed-session fresh Add.

The private-window observation corrects a test assumption: Firefox156 may display manager HTML but withholds extension APIs. Its toolbar/menu are unavailable and actual form submission creates no task, native download request or file. Do not describe `incognito: not_allowed` as preventing static document navigation. No signing, TLS, Safe Browsing, update, proxy or sandbox protection was weakened.

[FIREFOX_QUALIFICATION.md](FIREFOX_QUALIFICATION.md) records ownership-hardening findings, a failed driver initialization before Firefox launch, preserved evidence and explicit reviewed setup-only recovery. Normal harness cleanup still refuses unrecorded registration; a plausible path/addon ID is not adoption authority. Clean committed-driver/final-input tests and publication review remain required; no qualified release exists.
