...
Internal task format **v3 introduced** the required non-secret `needs_session` marker; current **v4** preserves it while adding the required nullable expected-checksum key. Strict v1/v2 migrations set it false because those formats never accepted session data. A recovered marked task cannot start a network probe or reuse retained bytes without its lost context. Missing v3/v4 markers fail validation; v3 migration preserves the marker. Exact original/final URLs remain protected recovery metadata, as before; this can include signed query data.
