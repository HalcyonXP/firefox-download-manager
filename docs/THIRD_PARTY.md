...
### Paired GUI system import review

The paired application/setup PE inventories add stock Windows Common Controls (`comctl32.dll`) through the existing reviewed winsafe0.0.29 GUI surface. Actual application imports are `DefSubclassProc`, `InitCommonControls`, `RemoveWindowSubclass` and `SetWindowSubclass`. These are OS-supplied control initialization/subclassing APIs, not bundled Microsoft DLL/SDK object redistribution; see [Common Control Versions](https://learn.microsoft.com/windows/win32/controls/common-control-versions). The local System32 library reports Microsoft Common Controls and a valid Authenticode signature; this is host observation, not package publisher authority. The first paired package attempt correctly failed the previous import allowlist. That exact stock import is now deliberately allowed, without permitting arbitrary development-mode imports for the paired recipe. Both binaries' static MinGW object inventories remain inside the existing reviewed policy. Companion-inclusive runtime/build dependency closure and notices are collected with the paired features; no new registry package, copied implementation or unsafe exception is added.

## Native Shell Link API surface

The paired setup's existing winsafe0.0.29 dependency additionally uses Shell Link COM (`CoInitializeEx`, `CoCreateInstance`, `IShellLink`, `IPersistStream::Save/Load`) and `IStream::Seek/Read` through its reviewed safe wrappers. The `shell` feature also supplies `SHCreateMemStream` from stock Windows `shlwapi.dll`; the checked local Windows file has a valid Microsoft Windows signature. No DLL or third-party implementation source is copied, and no additional registry package or first-party unsafe exception is introduced. `SHGetKnownFolderPath(Programs)` resolves the current-user anchor without a CREATE flag. Native stream serialization is followed by bounded Rust reads, exclusive writes and independent raw-target/arguments readback; no `Resolve`, shell execution or file-persist overwrite is used by setup. The first-party code does not call the dependency's `ISequentialStream::Write` wrapper. These SDK checks are not publisher authentication or installed launch qualification.
