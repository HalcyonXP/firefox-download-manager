...
`.github/workflows/ci.yml` separates debug quality checks from package qualification into independent `windows-latest` jobs, each with one Cargo build job and a 30-minute deadline. The quality job retains extension/protocol/manifest checks, Rust formatting/lint/tests/build, cancellation repetitions and packaging/qualification policy tests. The package job keeps BOTH clean-target builds on the SAME runner, together with release-target tests/repetitions, the native matrix/default 2 GiB fixture, byte comparison and isolated installation. No test or repeat count is removed. Only successful qualification uploads the candidate; Windows 11 x64-emulation waits for both jobs and consumes that same-run candidate. Dependency policy remains on Linux. These jobs do not publish a release.

The split addresses the measured cumulative budget failure in run34411956069: debug quality consumed about 13 minutes, candidate construction another 13, and the repeat build was cancelled at the 30-minute job limit. Installation and emulation did not run in that attempt. Splitting the independent phases preserves individual deadlines and same-runner repeatability semantics; it does not establish cross-machine reproducibility or a current-head pass. `scripts/ci-policy.test.mjs` structurally checks the retained gates and dependency wiring; hosted execution is still required.
