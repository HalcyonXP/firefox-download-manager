...
Project vocabulary and handoff decisions: [GLOSSARY.md](GLOSSARY.md).
...
The release target is a persistent **unsigned personal XPI** in Firefox Developer Edition, with existing browser settings and protections unchanged. Signed XPIs, Mozilla submission, signing credentials/accounts and marketplace listing are not requirements or release gates. Exact-artifact persistent installation and safe browser handoff remain unverified acceptance gates; temporary-addon reload, manual Add or mocked transport cannot substitute for the restart/click test. Owned isolated tests retain the safety boundaries in [ADR0014](decisions/0014-autonomous-completion.md), including closed-app/ownership preflights and no normal-profile inspection or modification.
...
- [#49 Prove persistent unsigned XPI installation and safe Firefox handoff](https://github.com/HalcyonXP/firefox-download-manager/issues/49)
...
#28's MIT and existing-machine decisions are preserved in draft PR #43 (not reverted on this independent main-based correction branch). A second computer or clean OS is not required; safety, privacy, ownership and exact-artifact gates remain.
...
The latest license-bearing local `26e302b` package passed twelve native checks and 2 GiB from a clean harness. The last actual Firefox run remains the clean pre-license `5ce837c` candidate slice, not a license-bearing rerun. Remaining actual controls/security/restart and artifact/adversary/report-safety/publication work is recorded in #28.

Merged-main CI `34333682602` subsequently passed all three jobs, as did integrated #28 CI `34334123810` at `f90f04c`. The expanded native artifact matrix passed 26 cases/2 GiB with a dirty identified driver. [NATIVE_QUALIFICATION.md](NATIVE_QUALIFICATION.md) preserves scope and the report-sink, constructor, adaptive-prefix and fixture-error corrections; clean-driver/new-tip qualification is still pending. Concurrent-normal-browser operation was outside that qualification scope. #28 remains In Progress/draft, with no tag/release.
...
## #50 transport foundation (2026-09-10)

Operational ownership/closed-app preflights remain required for integration testing. Signing credentials and publisher accounts are outside the unsigned personal distribution scope, not missing prerequisites.
...
Target local Firefox Developer Edition installations on Windows 11 with a persistent unsigned personal XPI. No signed XPI, Mozilla submission, publisher account, signing credentials or public AMO listing is required. Existing Developer Edition configuration may already permit unsigned persistent installation; do not inspect or change normal profiles or settings to establish that fact. Compatibility is distinct from observed persistence of exact Manager bytes. Preserve existing protections and qualify the install/restart/click path in owned isolated state. See ADR0013 and the definitions in GLOSSARY.md.
...
The paired development candidate now selects the application and compatible setup probe/UI together, preserving the eight payload roles and existing helper receipt digest. CLI/probe compatibility is not a shortcut ownership migration. Real-binary mock-registration lifecycle and native no-install UI checks are available; production selection remains refused. Next: owned actual GUI-to-tray/bridge handoff, explicit shortcut receipt/journal ownership, persistent unsigned XPI/capture and exact installed qualification. See COMPANION_CANDIDATE.md; this is not the end-user quick start.

The paired setup now implements the explicit receipt2/journal2 shortcut migration in [SHORTCUT_OWNERSHIP.md](SHORTCUT_OWNERSHIP.md). It creates a per-installation Start Menu entry, validates its generation copy, restores known previous bytes on interrupted activation and removes only matching entries on uninstall. Legacy default package/receipt1 behavior remains separate. Owned filesystem/COM and actual-application memory-registration cases are available; next is actual GUI/Start Menu-to-tray/bridge qualification, not another claim based on mock registration. Shared-registration closed-app gates and persistent unsigned-XPI acceptance criteria remain unchanged.

The first owned GUI→installed companion→native bridge→8 MiB output→Quit→uninstall slice passed with real registration and isolated state. Firefox/XPI and ordinary-click capture were not tested. The dirty development candidate and earlier failed driver domains are explicitly scoped in [INSTALLED_COMPANION_SLICE.md](INSTALLED_COMPANION_SLICE.md). A reusable opt-in installed driver now composes setup quiescence, retained native/fixture cleanup and guarded uninstall, with modeled lifecycle/failure tests; clean driverf0b538c exercised clean package1c49add for the native installed slice and nine expected-failure cases. Review then added an explicit nominal Manager exit-success assertion; clean harness16f755e subsequently passed the nominal installed sequence with that strengthened assertion against the same clean1c49add package. Next: qualify that driver, unsigned persistent Firefox integration, normal shortcut/physical tray interaction and exact-final-main qualification; #50 remains open.

## #49 browser API integration checkpoint

[Firefox capture API evidence](FIREFOX_CAPTURE_API.md) records an opt-in loopback-only probe maintained alongside #50. A fresh owned Firefox profile and unique temporary diagnostic XPI observed trusted direct/redirected attachment click correlation, asynchronous headers cancellation with NS_ERROR_ABORT, untouched navigation/POST/iframe cases, and an unarmed Firefox download with independently correct bytes. Browser/fixture retirement and final registration absence passed; no native registration was changed. The six-case dirty-harness report identifies its exact source digests and is explicitly non-qualifying for persistent installation or native handoff.

This does not select a production interceptor or unblock #51 acceptance. These API observations alone do not establish same-URL races, cross-origin/session/private/container behavior, native coordination or persistent unsigned Manager acceptance. Later sections record separate native and temporary installed-browser results. #49/PR55's earlier engine probe remains a separate branch/evidence layer; neither probe supplies the other's proof.

## Native handoff transaction checkpoint (#49/#50)

[NATIVE_HANDOFF.md](NATIVE_HANDOFF.md) defines the implemented Prepared/Committed/Aborted engine transaction and optional companion-only wire2 capability. Focused engine and actual local-pipe tests cover retained IDs, failed persistence, lost replies/reconnect and independently correct output. Manual Add and production capture selection remain unchanged. Browser pending-state persistence/UI and bounded cancellation/commit recovery components are implemented, with clean2df904b temporary-XPI nominal and missing-terminal integration observations; exact persistent unsigned installation/click qualification remains open. This component checkpoint does not close #49/#50 or unblock #51 acceptance; its source is separate from PR55's older engine diagnostic.

## Browser coordination component checkpoint (#49/#50)

[BROWSER_HANDOFF.md](BROWSER_HANDOFF.md) describes the implemented bounded journal, intent-before-cancel/observation-before-commit coordination, status recovery and pending presentation. The companion Hello compatibility mismatch is corrected in the extension and closed schema. A conservative click/request policy and browser registration adapter are unit-tested but unselected; the built manifest still has no mandatory site or webRequest authority. Only own-storage permission is added.

An identified dirty-source temporary loopback XPI passed the first owned installed Firefox-to-companion nominal slice and nine cleanup checkpoints; a later clean2df904b pairing passed nominal, missing-terminal confirmation and all nine cleanup checkpoints; [INSTALLED_BROWSER_SLICE.md](INSTALLED_BROWSER_SLICE.md) separates this from production/persistent qualification. Next: remaining recovery/history reconciliation and cross-origin/provider qualification before automatic activation, then persistent unsigned exact-XPI restart/click acceptance. No component test or older temporary API probe qualifies that workflow. CI `34545938464` at native checkpoint `9f1a3fc` passed quality/dependency policy but failed package qualification in three private-directory startups before observed readiness; emulation was skipped. No common cause, deadline relaxation or production fix is inferred.


### Phase-aware projection and recovery

Full native snapshots now distinguish durable handoff phase from transfer state through the additive `task_handoff_phase` capability. Dashboard and dispatch guards withhold inappropriate ordinary controls and retain committed handoff history. Older companion snapshots lacking phase remain unknown, not ordinary. Intent recheck can settle an already committed ID without replay; explicit continuation refuses a known aborted reservation before changing its journal stage. [BROWSER_HANDOFF.md](BROWSER_HANDOFF.md) and [PROTOCOL.md](PROTOCOL.md#task-phase-metadata) describe the compatibility and recovery boundaries. Clean2df904b new-pair nominal and missing-terminal confirmation tests passed with temporary XPI reload, and CI34567722468 passed all four jobs. Orphan/expiry reconciliation, actual cross-origin/provider and persistent unsigned final-artifact acceptance remain open; production capture stays unselected.


### Opt-in cross-origin redirect preparation

The unselected policy now requires each subsequent request to match the observed redirect target and its single Location, with fresh anonymous sent-header evidence, unchanged tab/document/default-store context and at most eight transitions. Cross-origin is off by default; an explicit option retains all anonymity and no-HTTPS-downgrade requirements. The temporary diagnostic scopes every hop to one or two independently owned loopback origins and adds `--scenario cross-origin`, reproducing the query/extensionless302/attachment-at-destination shape without selecting production capture. Clean1cde75b subsequently passed this actual installed-browser scenario plus nominal/missing-terminal regressions against unchanged paired2df904b, with independent output/fallback and joined retirement. It is a temporary two-port loopback result, not distinct-host/TLS, public Hugging Face or persistent-XPI acceptance. The earlier clean2df904b observations do not qualify these changed policy/diagnostic bytes.
...
Status-checked acknowledgement now resolves native-Aborted Cancelled/Confirmed notices, while an explicit bounded unlinked-reservation panel can discard Prepared tasks absent from loaded history. Neither action starts a transfer or erases retained identities. Same-ID recovery/capture guards, fixed warning/dispatch checks and a separate opt-in no-native-transfer browser driver are implemented; clean23bfa69 recovery observations now cover both scenarios against unchanged paired2df904b. The first attempt refused success reporting due to missing initial browser hash; a reproduced regression and retained comparison fix preceded the successful reruns. [BROWSER_HANDOFF.md](BROWSER_HANDOFF.md#explicit-stoppedunlinked-reservation-recovery) and [INSTALLED_BROWSER_SLICE.md](INSTALLED_BROWSER_SLICE.md#prepared-no-native-transfer-recovery-scenarios) define scope. Native history remains retained; no expiry or production-activation gate is silently waived.


### Capture preference implementation and remaining activation

A persistent default-on preference, actual manager checkbox and independent extension-only routing now distinguish registration availability from verified authorization. Off revokes new capture authorization immediately; On waits for strict write/readback. Pending handoffs/native history and existing transfers are untouched. Production registration/site authority remain unselected; only the existing owned loopback diagnostic activates this control. The modeled `capture-toggle` scenario covers armed Off→Firefox output, On→one native completion and persisted Off across reload without silently reapplying it. Clean4f5c006 now passes the six-case live batch against paired2df904b, including actual toggle/persisted-Off behavior and all related positive/recovery paths. Ordinary activation/permission approval, exact unsigned-XPI persistence and public Hugging Face/distinct-host/session acceptance remain open. See [the control contract](BROWSER_HANDOFF.md#persistent-capture-preference-and-activation-boundary) and [prepared scenario](INSTALLED_BROWSER_SLICE.md#prepared-capture-preference-scenario).


### Normal unsigned-XPI UI observation

The opt-in [persistence driver](FIREFOX_PERSISTENCE.md) serves the exact validated packaged manual XPI through an owned loopback link and uses ordinary visible installation buttons. It does not use temporary/direct installation APIs, signature overrides, profile injection or registration writes. Successful observation requires active/non-temporary/profile-scope identity, two successful retained browser exits, exact installed bytes and a restart with no installation call. A precisely observed signature failure is a distinct nonzero diagnostic outcome, not a new signing/account/settings requirement. The narrow current manifest, closed permission/source checks, no uncertain-click replay and retained cleanup have model coverage. A cleanbc36eb5 run against packaged2df904b observed the exact signature-required failure in owned defaults before approval, with CLI exit1, one successful browser exit/join and unchanged protections/absent registration. Persistence and approval UI were not reached. This environment-specific refusal neither determines normal-profile compatibility nor adds signing/settings-change requirements; exact persistence remains unverified.


### Unsupported browsing contexts: owned integration observations

The opt-in installed-browser `unsupported-contexts` scenario creates one owned container and one private window through Firefox's browser APIs inside the retained isolated process/profile. Armed default-On capture must leave both correct outputs to Firefox without native work; a following ordinary default-store click must still produce one native completion. Actual context attributes, nondefault container request metadata and denied private extension visibility are different required observations. No cookies/contextual-identities/private permission or preference override is added. Cleanff5e960 now passes this context scenario plus nominal, capture-toggle, cross-origin, missing-terminal, unlinked and aborted-terminal against unchanged paired2df904b. Actual container/private Firefox-only output, the following default-store native completion, all owner exits/joins/uninstall and15 independently reread files passed. Private-mode proof is distinct from private extension visibility (which remains denied); zero container identity is not default-cookie-store eligibility. See [source-scoped evidence](INSTALLED_BROWSER_SLICE.md#clean-context-and-regression-observations-ff5e960--paired2df904b). Production capture, provider/session/race/ordinary permission/persistence and exact-artifact gates remain open.
