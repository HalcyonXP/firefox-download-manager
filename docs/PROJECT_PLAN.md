...
The unselected policy now requires each subsequent request to match the observed redirect target and its single Location, with fresh anonymous sent-header evidence, unchanged tab/document/default-store context and at most eight transitions. Cross-origin is off by default; an explicit option retains all anonymity and no-HTTPS-downgrade requirements. The temporary diagnostic scopes every hop to one or two independently owned loopback origins and adds `--scenario cross-origin`, reproducing the query/extensionless302/attachment-at-destination shape without selecting production capture. This new scenario has model/owned-HTTP-fixture evidence only, not a live browser or Hugging Face result. The earlier clean2df904b observations do not qualify these changed policy/diagnostic bytes.
