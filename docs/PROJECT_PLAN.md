...
An opt-in [parent transport admission](FIREFOX_PROTECTION_BRIDGE.md#parent-transport-admission-opt-in-capture-disabled) entry and internal broker now distinguish connection admission from capture readiness. Real owned pipes exercised ordinary wire2, stale-admission refusal and one64KiB manual transfer after controller EOF; the actual NativeConnection passed a broker model. Capture stays disabled on that lane, and ordinary worker/application/package entry points do not select it. Selected production API/companion integration, registered request/handoff/context binding, revocation and complete applicable policy remain open. Automatic idle and whole-browser-active-native shutdown need separate evidence. Current component results do not qualify earlier or later package bytes.
