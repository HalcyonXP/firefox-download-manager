...
Clean driver `520f029` subsequently passed all 26 native cases/2 GiB on the same licensed CI candidate. Installer-driver review then extended the shared report/metadata/cleanup policy; twenty local harness tests pass, but the new installer boundary still needs actual lifecycle CI. See [NATIVE_QUALIFICATION.md](NATIVE_QUALIFICATION.md). Neither evidence update changes the remaining actual Firefox/publication gate.
