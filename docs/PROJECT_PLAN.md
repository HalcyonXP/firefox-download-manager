...
**Current implementation:** [CAPTURE_CANDIDATE.md](CAPTURE_CANDIDATE.md). Candidate eligibility is now loopback-only while the Firefox protection gap remains open. New Windows final publication establishes verified Internet-zone metadata without URLs; it is not a Firefox/antivirus verdict. The separately built [fileless privileged service probe](FIREFOX_PROTECTION_BRIDGE.md) passed at cleane1a982f with one nonblocking callback, typed synthetic-context getter consumption and caller/replay guards. The explicit disposable experiment-mode run also verified [corrected automation policy](FIREFOX_TEST_POLICY.md) and joined shutdown; the opt-in native engine now gates actual fingerprinted, exact-name publication with modeled one-use decisions and refuses protected execution after restart. An unselected parent transport component now retains startup/process/pipe/callback lifetimes and bounded frames under modeled tests. Separate authenticated peer classes and an exact private native-app entry now prevent caller JSON capabilities from selecting the parent connection class; ordinary dispatch explicitly refuses that class. An unselected fixed-host launcher now adds exact caller/nativeMessaging/manifest checks, own native-port keepalive and retained context/extension/shutdown handling under SDK-shaped models; these are not actual event-page or SDK-process observations. A separate [parent stdio fixture](PARENT_STDIO_FIXTURE.md) now provides opt-in native framing, a fixed diagnostic API, CLI-built SDK-shaped models and separately pinned archive validation. Five clean-e852421 standalone process cases passed with independent bytes/PID and retained waits/reader joins. A separate bounded observer/receipt client models exchange, native retirement and observer removal without retaining add-on API objects. A distinct default-off fresh-profile SDK controller now models exchange followed by explicit disable and native retirement before the exact outer Firefox join; independently supervised live execution and late browser-shutdown receipt delivery remain open. A separate Windows query/wait lease now distinguishes the retained launcher from its same-process or verified direct-child browser parent; owned Python fixtures exercise the handle boundary, and an initial live attempt reached direct-child browser admission but stopped before XPI loading. A retained compiler owner now prepares externally pinned archive expectations after Node exit and final readback; actual compiler/archive interop used non-executable image metadata. A retained failure supervisor now refuses premature finish and keeps unresolved owners across control-channel loss; owned Python process checks observed the hold and subsequent natural waits. Independently reviewed native-image provenance remains separate from archive integrity. No native SDK launch is established. A pre-load live refusal exposed incomplete platform environment/working-directory isolation and insufficient failure-stage receipts; corrections preserve the original guards. Completed Firefox SDK/controller integration, protected dispatcher/context revocation and browser policy remain unimplemented; no ordinary product entry selects the gate or parent transport. Permission/state/entry/build tests remain distinct from the earlier source-scoped browser observation.
...
| CI | CI34684148929 at6608962 and CI34681547342 ated0e64f passed all four jobs, before the new SDK run controller; this is not installed acceptance or an explanation of prior failures. CI34679554145 ate852421 failed quality in three private-directory cases before observed startup/request markers (23/26 passed; package/dependency passed, emulation skipped); retained waits/worker joins followed. CI34672412045 ate6bf502 failed packaging in three private-directory cases after observed startup/request write but before readiness (23/26 passed; quality/dependency passed, emulation skipped), before the native-port keepalive changes; retained process waits/worker joins were observed and no cause is established. Earlier CI34668388546 atd131796 passed all four jobs | Coordinator store-lock defect separately reproduced/fixed; unrelated intermittent adapter/bridge failures remain unexplained. No timeout/concurrency workaround |
