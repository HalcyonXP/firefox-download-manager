...
**Current implementation:** [CAPTURE_CANDIDATE.md](CAPTURE_CANDIDATE.md) remains loopback-only while the protection gap is open. Native Internet-zone metadata, full fingerprints and the opt-in exact-name publication gate do not establish Firefox policy. The clean-e1a982f fileless service probe observed only a synthetic-context nonblocking callback. The separate [parent stdio fixture](PARENT_STDIO_FIXTURE.md#observed-sdk-disable-slice) now has clean3c1ecfb SDK exchange/explicit-disable/native-retirement evidence with independent browser-parent/launcher/host waits; it does not establish persistence or registered companion lookup.

An opt-in [parent transport](FIREFOX_PROTECTION_BRIDGE.md#selected-parent-candidate-and-installed-driver) now has a fixed API, asynchronous NativePort adapter, separate0.3 manual candidate and explicit development companion routing. Real pipes cover selected/default routing, stale admission, one retained engine and independent manual bytes; the full client stack passes models. Capture remains disabled. The first clean ca33 installed-parent attempt reached manual completion and independently correct8MiB output but failed during add-on shutdown; SDK/browser joins and the second lifetime remain unverified. The diagnostic now separates its retained neutral control tab from the Manager tab and records precise bounded failure stages; this is modeled, not a runtime root-cause finding. [Failure cleanup](PARENT_DIAGNOSTIC_RECOVERY.md) now separates readback from failed qualification and retains the original setup authority until verified removal. Usable held-state presentation/control and a fresh supervised manual-lifetime slice remain next, followed by protected dispatcher/request/handoff/context binding, revocation and complete applicable policy. Separately, [Rust HTTP-fixture retirement](HTTP_FIXTURE_OWNERSHIP.md) now retains/joins handlers, and a received-before-close/recorded-after-close counterexample replaces remote ledger timing with local request-ownership assertions. The earlier CI failure remains undiagnosed. An unselected [handoff/context registry](FIREFOX_PROTECTION_BRIDGE.md#handoffcontext-association-unselected) now binds actual-reader metadata to immutable task/request/source identities with bounded history and revocation; the paired browser dispatcher and full policy remain unimplemented. Native scoped revocation permits a fresh browser-adapter context on the same engine receiver without reviving old handoffs or approved requests. An explicit, library-only parent2 route now carries native-origin challenges and scoped preparation/control/resolution with retained reader retirement; actual-pipe decisions remain modeled, ordinary APIs refuse private frames, and no worker/package selects it. Automatic idle, whole-browser-active-native shutdown and persistent unsigned acceptance remain separate gates. Default package selection remains manual-only; component results do not qualify changed package bytes.
...
| Parent SDK | Clean3c1ecfb fixed native exchange, explicit temporary-add-on disable, independently validated SDK retirement/collector removal and browser-parent/launcher/host waits | Fileless diagnostic only; no registered companion lookup, policy, automatic idle, whole-browser-active-native shutdown or persistence |
...
| CI | CI34697841701 at3c1ecfb failed quality (bridge9/10; last handoff observation Promoting/65536B,71 responses, worker joined and fixture retired) and packaging (setup23/26; two private-directory attempts before startup, one after request receipt; retained process waits/worker joins). Dependency policy passed; emulation skipped. Previous417953a CI34696247193 passed all four jobs | Distinct failures remain unexplained; no common cause or deadline/concurrency workaround is established. CI is not installed acceptance |
