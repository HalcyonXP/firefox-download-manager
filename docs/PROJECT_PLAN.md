...
### Normal unsigned-XPI UI observation prepared

The opt-in [persistence driver](FIREFOX_PERSISTENCE.md) serves the exact validated packaged manual XPI through an owned loopback link and uses ordinary visible installation buttons. It does not use temporary/direct installation APIs, signature overrides, profile injection or registration writes. Successful observation requires active/non-temporary/profile-scope identity, two successful retained browser exits, exact installed bytes and a restart with no installation call. A precisely observed signature failure is a distinct nonzero diagnostic outcome, not a new signing/account/settings requirement. The narrow current manifest, closed permission/source checks, no uncertain-click replay and retained cleanup have model coverage. This new slice has not run in Firefox; its eventual older-package result cannot qualify newer production bytes.
