...
An identified dirty-source temporary loopback XPI now passes an owned installed Firefox-to-companion nominal slice and nine cleanup checkpoints; [INSTALLED_BROWSER_SLICE.md](INSTALLED_BROWSER_SLICE.md) separates this from production/persistent qualification. Next: remaining live recovery cases and phase-aware UI/history handling before automatic activation, then persistent unsigned exact-XPI restart/click acceptance. No component test or older temporary API probe qualifies that workflow. CI `34545938464` at native checkpoint `9f1a3fc` passed quality/dependency policy but failed package qualification in three private-directory startups before observed readiness; emulation was skipped. No common cause, deadline relaxation or production fix is inferred.


### Phase-aware projection and recovery

Full native snapshots now distinguish durable handoff phase from transfer state through the additive `task_handoff_phase` capability. Dashboard and dispatch guards withhold inappropriate ordinary controls and retain committed handoff history. Older companion snapshots lacking phase remain unknown, not ordinary. Intent recheck can settle an already committed ID without replay; explicit continuation refuses a known aborted reservation before changing its journal stage. [BROWSER_HANDOFF.md](BROWSER_HANDOFF.md) and [PROTOCOL.md](PROTOCOL.md#task-phase-metadata) describe the compatibility and recovery boundaries. New-pair live tests, missing-terminal confirmation, orphan/expiry reconciliation, cross-origin eligibility and persistent unsigned final-artifact acceptance remain open; production capture stays unselected.
