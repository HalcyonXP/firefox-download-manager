...
The first owned GUI→installed companion→native bridge→8 MiB output→Quit→uninstall slice passed with real registration and isolated state. Firefox/XPI and ordinary-click capture were not tested. The dirty development candidate and earlier failed driver domains are explicitly scoped in [INSTALLED_COMPANION_SLICE.md](INSTALLED_COMPANION_SLICE.md). A reusable opt-in installed driver now composes setup quiescence, retained native/fixture cleanup and guarded uninstall, with modeled lifecycle/failure tests; its actual installed positive/fault executions remain pending. Next: qualify that driver, signed persistent Firefox integration, normal shortcut/physical tray interaction and exact-final-main qualification; #50 remains open.
