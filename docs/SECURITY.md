...
The source native-host manifest allows only `download-manager@halcyonxp.local`; installation substitutes one JSON-escaped absolute executable path and retains only Firefox's supported manifest fields. The extension requires `nativeMessaging` and `menus`, with no mandatory host grants; optional cookies and selected scheme/host grants support explicit session handoff. The old development registration scripts are retired refusal-only stubs. Rust setup uses verified immutable generations, strict receipts/journals, ordinary ancestor leases and a cooperative application-data-domain lock. It writes only the fixed 64-bit current-user `HKCU\Software\Mozilla\NativeMessagingHosts\com.halcyonxp.firefox_download_manager` registration after a bounded isolated helper hello and copied-byte rechecks. Foreign, malformed, changed and unknown entries are preserved/refused; uninstall/cleanup remove only proven generated files, not task state or downloads. It creates no service, listener, scheduled task, firewall rule, VPN setting or route and never modifies Firefox's profile.

The project has no remote updater. Explicit local package upgrade replaces registration, not a running binary. S26-4's historical development-script findings and #27 remediation evidence are distinguished in [PACKAGING_PLAN.md](PACKAGING_PLAN.md). Directory leases and exact registry rechecks are not a hostile-account sandbox or atomic filesystem/registry power-loss transaction; conservative failures may require owner inspection. First-party licensing is MIT under ADR 0011; third-party runtime/compiler terms remain independent. Neither a license nor package checksums authenticate a publisher. Final #28 qualification remains separate.
