...
Declare a stable Gecko extension ID because the native-host manifest must allow exactly that ID. Request only permissions justified by implemented behavior. At the native-host stage (#18), the sole permission was `nativeMessaging`. Explicit capture (#19) added `menus`; that stage requested `nativeMessaging` and `menus` with no host permissions. #23 subsequently added optional cookies/selected-site grants under ADR 0010; #26 adds explicit CSP/private-window/minimum-version restrictions documented in [the security review](../SECURITY_REVIEW.md). The native connection remains dormant until a manager surface asks for it, and each new port is usable only after hello plus an atomically assembled complete snapshot. The original deferral of cookie/site authority was resolved by #23, not by a blanket mandatory grant.
