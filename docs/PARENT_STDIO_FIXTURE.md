...
This is a separate diagnostic for eventual Firefox SDK process/pipe/lifetime observation, **not a Manager controller, protected dispatcher or install candidate**. One source-identified SDK exchange/explicit-disable slice is observed below; it is not installation acceptance. The normal extension, capture candidate and paired package do not select it.
...
| Process leases | `scripts/qualification/process_lease.py` | Separate noninheritable query/wait handles for the retained launcher and same-process or verified direct-child browser parent; no termination rights |
| SDK disable controller | `scripts/qualification/parent_run.py`, `parent_control.js` | Separately opt-in fresh-profile exchange/disable orchestration, exact retained Firefox owner and refusal-preserving cleanup; live slice below, no CLI or compiler |
...
Eleven compiler-owner models and eleven rejected/restored mutations cover fixed inputs/environment, one-shot start, unknown start, actual wait/type checks, timeout followed by later join, final rereads and output revocation. A real retained Node invocation also built and independently reread an exclusive five-payload XPI using deliberately **non-executable native metadata bytes**, with natural Node exit0. That dirty-source interop domain is private and must never be executed, relocated, relabeled clean or published as a candidate. No Firefox or native fixture was launched. The separate failure supervisor below retains unresolved owners; this build-only interop does not exercise SDK execution. Original browser/registration preflights remain prerequisites for later SDK execution, not for this build-only component.
...
The first case is ordered: retained Firefox launcher and independently acquired same/direct-child process lease → validated policy → independent observer installation → fixed-ID absence check → one temporary load → active fixed-ID temporary background metadata → one echoed exchange → one explicit add-on disable → successful SDK retirement receipt → checked observer removal → policy readback → exact Firefox launcher wait/exit0 and independent process-handle waits/release → final preflights/input checks → exclusive nonqualifying report. The fixed control script accepts only `absent`, `info` or `disable`, never arbitrary add-on IDs, enable/uninstall or native data. Disable completion and disabled metadata are not native retirement.
...
At `2b291fe`, eighteen Python controller models, eight actual-source control-script models and seventeen rejected/restored mutations cover ordering, exact modes/types, consumed domains, existing profiles, early/late faults, uncertain commands, interruption during cleanup, unknown native/outer lifetime, process replacement and nonzero exit. They execute no browser, native image or registry operation. Those controller models are not Firefox execution. The later clean3c1ecfb slice below separately observed SDK exchange and explicit-disable retirement; automatic event-page idle remains unobserved. Whole-browser shutdown with an active native process is deliberately not this case; its late-receipt strategy remains open.
...
The controller now retains `ProcessLease` before acquisition and before any diagnostic XPI load. It keeps the original `Popen` object unchanged and permits only its same PID or a direct child identified by the owned chrome control channel. For a different PID, a Toolhelp parent check must pass before opening process handles and must still pass after acquisition. Both handle IDs and executable paths must match; the child creation time cannot precede its retained live launcher. Launcher liveness is checked before and after acquisition, and already-signaled handles refuse admission. This replaces equality-only refusal with an explicit ownership check, not arbitrary PID adoption. The initial live attempt observed direct-child admission but stopped before XPI loading. A separate clean3c1ecfb run below completed the SDK exchange/disable slice.

The backend uses only `SYNCHRONIZE | PROCESS_QUERY_LIMITED_INFORMATION`, with noninheritable handles. It does not borrow/cast a `Popen` private handle, request termination/write/memory rights, disable the Firefox launcher, change mitigations or inspect normal profiles. The Toolhelp snapshot necessarily contains system process records; traversal reads PIDs and the selected parent field without retaining or logging unrelated fields or opening unrelated process handles. The original profile, executable-hash and closed-app/registration checks remain mandatory. PID, parent, creation-time and path consistency are **not authentication against a hostile same-user process**, executable-byte provenance or authority to attach to a caller-selected process.

`observe()` is nonblocking: only a signaled retained handle followed by its actual exit-code query records a process wait. Child exit, launcher exit, original `Popen.wait()`, native SDK retirement and reader joins remain distinct. Every acquired process handle must be observed and released; all exit codes must be zero for a successful lease receipt. Failed acquisition retains partial handles for cleanup. Snapshot inverses are also retained on failure, even when no process handle was opened. An uncertain close is never replayed. Cancellation remains cancellation after cleanup errors; two models first exposed inverse failures masking it. Another controller regression exposed an outstanding snapshot being overlooked when the process-handle list was empty; aggregate cleanup now includes the snapshot owner.

The success report adds `launcher_pid` and `browser_parent_owner`; `browser_pid` identifies the process reported by the owned chrome channel. The legacy `browser_joined`/`browser_exit` fields retain their outer-launcher meaning rather than silently redefining historical receipts. The additional process-owner receipt is mandatory before success. No receipt exposes paths, process handles, command lines or memory. An active or uncertain parent lease prevents success even after the launcher exits.

Thirteen targeted mutations were rejected after restoring and checking the positive baseline; source bytes were restored. Fault injections require an exact phase/occurrence tuple so an inert injection cannot masquerade as a negative control. Twelve lease/SDK-inverse models and twenty-three controller models cover partial acquisition, old/foreign/replaced metadata, actual signaling versus timeout, nonzero exit, uncertain close, cancellation and early/late cleanup. Separate owned Python process fixtures observed same-process leasing, direct-child exit while its launcher remained live, and sibling rejection before process-handle acquisition. All fixture processes and the direct-child reader were joined naturally. These are Windows handle-component observations, **not Firefox, native-fixture SDK or persistent-install evidence**. The initial live attempt reached browser startup but not XPI loading; the later clean3c1ecfb batch below supplies separate SDK evidence. Historical output/UI observations and waited outer launchers remain source-scoped and are not relabeled direct browser-parent-handle joins.

The Windows ABI was checked against SDK10.0.26100.0 headers; 64-bit structure size/offsets are checked before use. Relevant platform contracts: [PROCESSENTRY32W](https://learn.microsoft.com/windows/win32/api/tlhelp32/ns-tlhelp32-processentry32w), [OpenProcess](https://learn.microsoft.com/windows/win32/api/processthreadsapi/nf-processthreadsapi-openprocess), [GetProcessTimes](https://learn.microsoft.com/windows/win32/api/processthreadsapi/nf-processthreadsapi-getprocesstimes), [QueryFullProcessImageNameW](https://learn.microsoft.com/windows/win32/api/winbase/nf-winbase-queryfullprocessimagenamew), and [WaitForSingleObject](https://learn.microsoft.com/windows/win32/api/synchapi/nf-synchapi-waitforsingleobject).
...
## Initial live refusal and environment containment

A clean417953a attempt prepared the archive and started an owned Firefox instance, with distinct launcher/browser-parent IDs and a removed independent observer. It refused before XPI loading. Failure cleanup recorded the outer browser wait/exit0 and released parent handles; no successful lease receipt or native SDK invocation was recorded. The outer host was waited and its reader joined before its required-exit0 assertion failed. Its numerical exit was not persisted independently; the worker reported exit1. These observations are not exchange, explicit-disable or installation acceptance.

Unexpected literal `%SystemDrive%/ProgramData` output appeared in the checkout during that attempt. Fileless Windows expansion controls reproduce an unresolved placeholder when `SystemDrive` is absent from the child environment. The private host had omitted that platform variable; the browser also inherited the repository working directory. This does not establish every write's producer or the exact pre-load refusal branch. The browser launcher now selects its owned profile as `cwd`, and the pre-load input/absence stages are distinct. A model first reproduced the missing working-directory selection. Platform variables must retain their normal Windows values rather than be omitted by a minimal host environment. No protection preference or deadline is relaxed. A working directory contains relative output; it is not a sandbox or a guarantee against absolute-path writes. Failed artifacts remain non-qualifying.

## Observed SDK disable slice

Clean driver/archive source `3c1ecfbeb2c50db21a6566ecd8d80e059face79e` completed one independently supervised fileless SDK exchange and explicit temporary-add-on disable. The native image had separate retained clean-e852421 build provenance; archive integrity did not substitute for that build. Original fresh app/registration preflights, pinned inputs and the exact experiment-mode protection tuple were checked before execution and after retirement.

| Identity | SHA-256 |
| --- | --- |
| Native fixture image | `b8c635cc822d87afe837de6d631cb4cd8078aba1cd5346deebf3b99adf825b74` |
| Diagnostic XPI | `8fbfad2d8f2ac1b5c09ec21805d4f3b2288df03d1e0d6fa5a58a8555d192211d` |
| Firefox executable | `8a6a2339da19ccd55be1832583d6ed83a7f25c03a3fab5fe5af52ebd2e68708c` |
| `artifacts/parent95-sdk-disable.json` | `ab11e13bed707631493a423018c81bcbe20a27b6cbb49377c705b18d44b8293b` |

The independent collector observed the fixed UTF-8 exchange and validated SDK retirement after disable, before browser close. That receipt requires the retained process exit0, actual three-pipe closure, settled component I/O/callbacks and removed hooks without forced retirement. Collector removal passed its positive/negative controls. The distinct browser parent and launcher had two independently retained query/wait handles, both signaled, queried exit0 and released; the original launcher was also waited. The outer diagnostic host was independently waited at exit0 and its reader joined, with its numerical exit persisted before success assertions. Final archive/report hashes and app/registration absence were independently reread.

The report remains `qualification:false`, `m5_install_ready:false`, `temporary_loading_used:true`, and `browser_shutdown_with_active_native_qualified:false`. No registered native lookup, companion IPC, download-data file, scan or publication occurred. Earlier failed and metadata-only domains remain separate evidence; this run does not repair their missing observations or qualify later changed source.

## Remaining live selection

Automatic event-page idle and whole-browser shutdown with active native work remain distinct unobserved cases. They require separately reviewed observation/ownership strategies, not reuse of the consumed disable controller. Future batches still need exact source/image ownership, fresh domains and original preflights. Unknown startup or failed cleanup must preserve the domain and refuse success.

Archive integrity and standalone fixture evidence do not qualify persistent installation, automatic event-page idle lifetime, private helper readiness, one-controller integration, request/handoff/context association, complete policy enforcement or real-file publication. Those remain requirements in [FIREFOX_PROTECTION_BRIDGE.md](FIREFOX_PROTECTION_BRIDGE.md) and [PROJECT_PLAN.md](PROJECT_PLAN.md).
