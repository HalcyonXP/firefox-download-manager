...
The driver uses two distinct temporary-XPI lifetimes, fixed-ID checks, independent bounded retirement observations and separate browser-parent/launcher waits. Its reports retain the selected policy snapshot and label `profile_mode: parent-transport-experiment`, `qualification:false`, and `capture_ready:false`. The first clean ca33 live attempt reached manual completion but failed during shutdown; it does not establish installed-lifetime or persistent-XPI acceptance. The current diagnostic separates a retained neutral control tab from the extension-owned Manager tab and returns to that exact handle before disable, without changing close-last-tab preferences. Fixed-stage/allowlisted-error records do not substitute for SDK/process joins. Selected [cleanup-only readback](PARENT_DIAGNOSTIC_RECOVERY.md) is distinct from failed qualification, retains the same process/sandbox/collector and never resets corrupt parsed evidence or repeats an uncertain inverse. The original setup witness remains live while browser cleanup/removal is pending. This does not relax preflights, settings or normal-profile boundaries. See [the selected parent candidate and driver](FIREFOX_PROTECTION_BRIDGE.md#selected-parent-candidate-and-installed-driver).
