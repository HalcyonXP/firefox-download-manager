...
- **Browsing container**: a Firefox contextual identity with a nonzero `userContextId` and a nondefault cookie store; it is not a private window. The diagnostic reports only `default`/`other`/`missing` store classes, not store identifiers. Automatic capture remains default-store-only.
