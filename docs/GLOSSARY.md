...
- **Fixed-host parent launcher**: unselected owner validating the exact background caller, nativeMessaging permission and own native manifest before one SDK invocation. Transport startup is not native private readiness or policy authority. Refused read-only lookup can still retire cleanly without invocation; a rejected SDK call cannot imply no child. Failed retirement retains its shutdown failure observation. See [FIREFOX_PROTECTION_BRIDGE.md](FIREFOX_PROTECTION_BRIDGE.md#fixed-host-launcher-unselected-parent-component).
...
- **Parent transport retirement observation**: the unselected transport separately records startup certainty, retained SDK process exit, actual pipe closure and its own I/O/callback completion. Disconnect, response queueing and a retirement timer are not joins. A successful transport observation does not establish descendant-worker, companion or service retirement, policy correctness or publication. See [FIREFOX_PROTECTION_BRIDGE.md](FIREFOX_PROTECTION_BRIDGE.md#parent-transport-lifetimeframing-component-unselected).
