...
- **InstalledImage / runtime candidate (#50)**: a retained running-image, current receipt/generation/file and registration binding is separate from engine state-lock ownership. Runtime record1 must match that independent binding; a candidate filename or stored capability alone does not prove liveness. One installed image may implement separate companion/bridge processes; the next deployment has not yet selected those modes.
