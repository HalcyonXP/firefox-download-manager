...
- **Fixed-name binding / named validation owner**: `NamedValidatedPartial` holds one immutable sanitized/numbered filename with a still-owned fully hashed validation lease. It attempts only that component; collisions do not authorize an alternate. Neither a directory reservation nor a reputation verdict. Drop/refusal requires fresh validation before another binding; ordinary promotion remains separately auto-numbering.
...
- **Browsing container**: a Firefox contextual identity with a nonzero `userContextId` and a nondefault cookie store; it is not a private window. The diagnostic reports only `default`/`other`/`missing` store classes, not store identifiers. Automatic capture remains default-store-only. A zero `userContextId` (the context report's `container:default`) alone does not establish default-cookie-store eligibility: private mode remains a separate condition.
...
- **Capture candidate / diagnostic / default build**: the capture candidate is an explicitly built development XPI using actual startup/UI with reviewed site authority and no arming or fault controls. The candidate is currently restricted to loopback test origins pending Firefox download-protection integration; its broader declared permissions do not authorize public capture. The diagnostic is an owned-loopback test instrument. The default/manual build selects neither automatic entry nor site authority. None of these names alone establishes release acceptance.
...
- **Windows Internet-zone provenance / MOTW**: this project uses the `Zone.Identifier` alternate data stream, not an HTML Mark-of-the-Web comment. New native Windows publication writes/readbacks fixed Internet-zone3 (or preserves canonical Restricted-zone4) before final linking. No URL/session data is stored. This metadata is not a Firefox reputation/antivirus verdict or protection-parity qualification; see [SECURITY.md](SECURITY.md#windows-download-provenance).

- **Protection API experiment / fileless service probe**: privileged Firefox-parent code with a closed no-argument service-observation surface, separately built and unselected by product manifests. Its fixed empty-loopback query has no native-file or publication authority. Experiment availability, service response, complete protection integration and persistent installation are different observations. See [FIREFOX_PROTECTION_BRIDGE.md](FIREFOX_PROTECTION_BRIDGE.md).

- **Handoff/context binding and context epoch**: an internal memory-only association between one handoff UUID and actual registered request/tab/source metadata. Its opaque binding object is scoped to one caller/registry; its monotonic numeric epoch is local, not a native admission ID, engine generation or protection-receiver lifetime. A snapshot is historical metadata, not cancellation/policy/publication authority. See [the unselected association](FIREFOX_PROTECTION_BRIDGE.md#handoffcontext-association-unselected).
- **Parent2 private dispatcher**: opt-in native library route with its own version2 offer/accept/ready and scoped `protection_bridge:1` frames. It transports original native challenges but does not establish Firefox identity, policy correctness or capture readiness. Existing parent1/default entries and extension-caller send APIs refuse it; no package selects it.
- **Native protection context**: one noncloneable, revocable child owner of an engine's retained protection receiver. A scoped handoff pins this owner, not a caller-supplied UUID. Its close/drop invalidates even approved publication under the receiver's final-publication lock; a new context cannot revive old tasks. Context revocation is not transfer/process joining. `context_id()` differs from the receiver UUID returned by the legacy `connection_id()` accessor and from browser-local metadata epochs.
- **Required-protection handoff**: opt-in native envelope6 record that cannot silently execute/publish as an ordinary handoff. It stores a requirement, not browser context or a verdict; recovered execution is refused. No product/wire entry selects it yet.
- **Protection request / receiver lifetime**: native-origin, one-use decision route bound to a still-owned fingerprinted exact-name lease, immutable handoff ID, run counter, request UUID and one receiver UUID. Receiver retention is not Firefox readiness; response queuing is not publication or process join. See [FIREFOX_PROTECTION_BRIDGE.md](FIREFOX_PROTECTION_BRIDGE.md#native-publication-gate-opt-in-engine-component).

- **Fixed-host parent launcher**: internal owner selected by the separate manual parent candidate, validating the exact background caller, nativeMessaging permission and own native manifest before one SDK invocation. Transport startup is not native private readiness or policy authority. Refused read-only lookup can still retire cleanly without invocation; a rejected SDK call cannot imply no child. Failed retirement retains its shutdown failure observation. See [FIREFOX_PROTECTION_BRIDGE.md](FIREFOX_PROTECTION_BRIDGE.md#fixed-host-launcher-unselected-parent-component).
- **Native-port keepalive / retirement guard**: exact-owned SDK native-port registration suppresses automatic event-page idle termination, not explicit shutdown. Its scheduling tag proves neither process creation nor browser/policy authority. The launcher retains that entry and its AsyncShutdown failure observation through retirement; uncertain registration/removal or native retirement refuses success. This does not join the process or shared SDK worker by itself.
- **Fileless parent stdio fixture**: separate no-download-data/no-scan diagnostic with executable/build inputs; not the fileless reputation-service probe. Fixed argv and a PID-correlated ping/pong supply no Manager/private-IPC/policy authority. Its `echoed` record describes an exchange, not current private readiness; `retired` additionally requires retained lifetime observations. Both remain `qualification:false`. See [PARENT_STDIO_FIXTURE.md](PARENT_STDIO_FIXTURE.md).
- **Parent fixture build expectation**: values supplied by a separately retained compiler controller: exact domain, commit/dirty state, nonce and image/build-record hashes. Archive validation pins those values and an external archive hash; it does not prove compiler joins, native authority or persistent installation. The domain cannot change because the bundle embeds absolute native paths. See [PARENT_STDIO_FIXTURE.md](PARENT_STDIO_FIXTURE.md).
- **Parent fixture collector / collector ID**: an observer in one controller-owned named chrome sandbox, retaining at most two original bounded JSON records rather than add-on API objects. Its fresh collector ID is distinct from the fixture nonce; neither authenticates Firefox or native policy. Positive delivery control and post-removal non-observation concern this observer only, not native or outer-browser joins. See [PARENT_STDIO_FIXTURE.md](PARENT_STDIO_FIXTURE.md#independent-observation-boundary).
- **Parent transport retirement observation**: the retained transport separately records startup certainty, retained SDK process exit, actual pipe closure and its own I/O/callback completion. Disconnect, response queueing and a retirement timer are not joins. A successful transport observation does not establish descendant-worker, companion or service retirement, policy correctness or publication. See [FIREFOX_PROTECTION_BRIDGE.md](FIREFOX_PROTECTION_BRIDGE.md#parent-transport-lifetimeframing-component-unselected).

- **Parent SDK disable run**: the separately opt-in `parent-stdio-sdk-disable` diagnostic case: one fixed exchange, explicit temporary-add-on disable, independently received SDK retirement and observer removal, then exact outer Firefox join. It does not test late receipt delivery during whole-browser shutdown with active native work, idle lifetime, persistence or Manager readiness. See [PARENT_STDIO_FIXTURE.md](PARENT_STDIO_FIXTURE.md#sdk-exchangedisable-controller).
- **Outer Firefox launcher / browser parent**: the retained process used to start Firefox and the process identified by `Services.appinfo.processID`, respectively. On Windows these can be distinct. A launcher forwarding its child's exit status does not give the external controller that child's retained OS handle. Neither is the SDK native fixture process. Same-PID observations must be established, not assumed. See [PARENT_STDIO_FIXTURE.md](PARENT_STDIO_FIXTURE.md#outer-launcher-versus-browser-parent).

- **Windows process lease (diagnostic)**: a retained noninheritable query/wait handle for an owned launcher and, where different, its verified direct child. Metadata consistency precedes use; actual signaling, exit-code query and handle release precede a receipt. Snapshot cleanup is also owned. This is neither a PID-only adoption rule nor browser authentication; it does not terminate processes or replace the original launcher/native/reader joins. See [PARENT_STDIO_FIXTURE.md](PARENT_STDIO_FIXTURE.md#outer-launcher-versus-browser-parent).

- **Parent API connection token / parent candidate**: an API-local monotonic integer correlates one retained manual connection, not a native admission UUID, task or browser-policy epoch. The separate0.3 candidate selects the fixed SDK API at build time and never falls back to ordinary Native Messaging when it is unavailable. Capture remains disabled; temporary-driver and persistent-install evidence are separate.
- **Owned neutral control tab**: the original `about:blank` tab retained by the installed-parent diagnostic, separate from its extension-owned Manager UI tab. Disable and retirement observations use its exact handle so extension-tab removal does not remove their selected context. It is not policy authority or an idle-keepalive substitute; missing/changed handles refuse observation rather than being rediscovered.
- **Parent transport admission / admission ID**: opt-in native offer → matching accept → matching ready on one class-checked connection, before ordinary wire2. The native-generated UUIDv4 is ephemeral correlation, not a task, browser authentication, protection-receiver lifetime or captured-context epoch. Current `capture_ready:false` never authorizes cancellation/publication. See [FIREFOX_PROTECTION_BRIDGE.md](FIREFOX_PROTECTION_BRIDGE.md#parent-transport-admission-opt-in-capture-disabled).

**HTTP fixture retirement**: stop acceptance, unblock owned fixture barriers/I/O/stalls, then join every retained handler and the listener. Socket shutdown, request-ledger insertion and completed joins are different observations. Guard release can permit response delivery; retiring the server does not promise delivery. See [HTTP fixture ownership](HTTP_FIXTURE_OWNERSHIP.md).

**Parent cleanup observation**: a separate, same-collector monotonic tracker for failure-only retirement readback. It cannot issue qualification receipts or clear original failures. A read gap is not parsed corruption, and removed-after-readback is not an earlier removal call returning. See [diagnostic recovery](PARENT_DIAGNOSTIC_RECOVERY.md).

**Diagnostic control sequence**: a bounded local-host request ordinal, consumed before dispatch and echoed in phase/status responses. It is not native wire2, a task/admission identity or retirement authority. Failed-held continuation preserves original owners and failures; it does not restart the diagnostic. See [retained control channel](PARENT_DIAGNOSTIC_RECOVERY.md#retained-control-channel).
